...
public:
    ...
    vector<int>* additem(T n) { // обертка для добавления элемента
        ...
        tree_function_objects_names.push_back(n->get_performer());
        return path;
    }
...
public:auto searchOrderTraversWrapper(string name)
{
    ...
    public: auto tree_search(T object)
	{
		...
		if (head != NULL)
		{
			while (head != nullptr)
			{
                if (head->num->get_role() == object->get_role())
                {
                    node* tmp = head;
                    while (tmp != nullptr)
                    {
                        if (tmp->num->get_name() == object->get_name() && tmp->num->get_role() == object->get_role() &&
                            tmp->num->get_performer() == object->get_performer())
                            return true;
                        else
                            tmp = tmp->next;
                    }
