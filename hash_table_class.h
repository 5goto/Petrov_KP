...
enum class status // состо€ни€ €чейки
{
...
template <typename T>
class SimpleHashTable  //  онтейнер - хэш таблица
{

	struct cell  // €чейка таблицы
	{
		T data;
		status cell_status;
...
protected:
	...
	public:
		NodeIterator(const NodeIterator& it) : value{ it.value } {} // конструктор копировани€ 
...
public:
	...
	int delete_element(T item)
	{
		...
		if (index_to_delete != -1)
		{
			...
			return -1; // не найден удал€емый ключ
	}
};
